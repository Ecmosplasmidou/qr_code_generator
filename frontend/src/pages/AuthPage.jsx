    ...
    <main className="relative min-h-screen flex items-center justify-center px-4 py-20 overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        <img 
          ...
          alt="Background" 
          className="w-full h-full object-cover opacity-20 mix-blend-multiply pointer-events-none"
        />
      ...
      {/* AUTH CARD */}
      <div className="relative z-10 bg-white/80 backdrop-blur-2xl p-8 md:p-12 rounded-[3.5rem] shadow-[0_30px_80px_-15px_rgba(0,0,0,0.1)] border border-white w-full max-w-[480px] space-y-8 animate-in fade-in zoom-in-95 duration-700">
        
        {/* HEADER ICON & TITRES INTUITIFS */}
        <div className="text-center space-y-5 mb-2">
          ...
          <div>
            <h2 className="text-3xl md:text-4xl font-black uppercase italic tracking-tighter text-slate-900 leading-none">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button className="w-full bg-slate-900 text-white mt-2 py-6 rounded-2xl font-black uppercase text-[11px] tracking-[0.2em] flex items-center justify-center gap-3 hover:bg-blue-600 hover:-translate-y-1 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.2)] hover:shadow-blue-500/40 transition-all duration-300 active:scale-95">
            {isLogin ? 'Accéder au Studio' : 'Créer mon accès PRO'} <ArrowRight size={18}/>
          </button>
        </form>
        ...
        <div className="space-y-6 pt-2">
          <div className="relative">
            ...
            <div className="relative flex justify-center">
              <span className="bg-white px-4 text-[9px] font-black uppercase text-slate-300 tracking-[0.3em] rounded-full">ou continuer avec</span>
            </div>
          </div>

          <button onClick={() => window.location.href='#'} className="w-full flex items-center justify-center gap-4 bg-white border border-slate-200 py-4.5 rounded-2xl font-bold text-xs text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-all shadow-sm group">
            <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" className="w-5 group-hover:scale-110 transition-transform" alt="Google" /> 
            Google Workspace
          </button>
        </div>
        <div className="mt-3 text-slate-500 text-sm font-medium flex justify-center">
          {isLogin ? (
            <p>Nouveau sur QRLYZE ? <button type="button" onClick={() => setSearchParams({ mode: 'register' })} className="text-blue-600 font-black hover:underline underline-offset-4 transition-all ml-1">Créer un compte</button></p>
          ) : (
            <p>Déjà membre ? <button type="button" onClick={() => setSearchParams({ mode: 'login' })} className="text-blue-600 font-black hover:underline underline-offset-4 transition-all ml-1">Se connecter</button></p>
          )}
        </div>
      </div>
