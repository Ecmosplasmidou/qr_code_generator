    ...
    try {
      ...
      if (res.ok) {
        if (isLogin) {
          ...
          localStorage.setItem('isPro', data.is_pro);
          navigate('/app');
        } else {
          setIsLogin(true);
          setNotif({ type: 'success', msg: 'Compte créé ! Connectez-vous.' });
        }
      } else { setNotif({ type: 'error', msg: data.message }); }
    } catch { setNotif({ type: 'error', msg: 'Erreur serveur' }); }
  };
    ...
    <main className="min-h-screen flex items-center justify-center px-4 py-20 bg-slate-50">
      ...
      <div className="bg-white p-8 md:p-14 rounded-[3rem] shadow-2xl w-full max-w-[450px] space-y-8">
        <div className="text-center space-y-2">
          <div className="bg-blue-600 text-white w-12 h-12 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-blue-100"><ShieldCheck size={28}/></div>
          <h2 className="text-3xl font-black uppercase italic tracking-tighter">{isLogin ? 'Connexion' : 'Inscription'}</h2>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <input type="email" required placeholder="Email" className="w-full px-6 py-4 rounded-2xl bg-slate-50 outline-none font-bold shadow-inner" value={email} onChange={e => setEmail(e.target.value)} />
          <div className="relative">
            <input type={showPassword ? "text" : "password"} required placeholder="Mot de passe" className="w-full px-6 py-4 rounded-2xl bg-slate-50 outline-none font-bold shadow-inner" value={password} onChange={e => setPassword(e.target.value)} />
        ...
        <div className="relative"><div className="absolute inset-0 flex items-center"><div className="w-full border-t border-slate-100"></div></div><div className="relative flex justify-center text-[9px] font-black uppercase text-slate-300 bg-white px-2">Ou</div></div>
        <button onClick={() => window.location.href='#'} className="w-full flex items-center justify-center gap-3 bg-white border border-slate-200 py-4 rounded-2xl font-bold text-slate-600 hover:bg-slate-50 transition-all">
          <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" className="w-5" /> Continuer avec Google
        </button>
        <button onClick={() => setIsLogin(!isLogin)} className="w-full text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-blue-600">{isLogin ? "Créer un compte" : "Se connecter"}</button>
      </div>
    </main>
  );
};
